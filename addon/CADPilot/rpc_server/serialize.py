...
def serialize_view_object(view):
    ...
    _try_prop(result, view, "ShapeColor", serialize_value)
    _try_prop(result, view, "Transparency")
    _try_prop(result, view, "Visibility")
    # Extended visual properties
    _try_prop(result, view, "DisplayMode")
    _try_prop(result, view, "LineColor", serialize_value)
    _try_prop(result, view, "PointSize")
    _try_prop(result, view, "LineWidth")
    _try_prop(result, view, "DrawStyle")
    return result


def _try_prop(result, view, name, transform=None):
    """Safely read a ViewObject property, applying an optional transform."""
    try:
        value = getattr(view, name)
        result[name] = transform(value) if transform else value
