...
# MCP server parameters (fetches cadpilot from PyPI on first run).
# Dev mode (local clone): command="uv",
#     args=["--directory", "/path/to/cadpilot", "run", "cadpilot"]
server_params = StdioServerParameters(command="uvx", args=["cadpilot"])
    ...
    async with stdio_client(server_params) as (read, write):
        ...
        async with ClientSession(read, write) as session:
            ...
            while True:
                ...
                messages = [SystemMessage(content=INSTRUCTION), HumanMessage(content=user_input)]
                try:
